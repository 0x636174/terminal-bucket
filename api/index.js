...
const getParentCommentData = async (requestOptions, comment_id) => {
  const { data } = await bitbucket.pullrequests.getComment({
    ...requestOptions,
    comment_id,
  });
  return data;
...
const createComment = async (commentOptions) => {
  const { data } = await bitbucket.pullrequests.createComment({
    ...commentOptions,
  });
  return data;
};
...
const getPrActivity = async (requestOptions, prId) => {
  const { data } = await bitbucket.pullrequests.listActivities({
    ...requestOptions,
...
const getDiffStat = async (requestOptions) => {
  const { data } = await bitbucket.pullrequests.getDiffStat({
    ...requestOptions,
  });
  return { data };
};
...
const approvePr = async (requestOptions) => {
  const { data } = await bitbucket.pullrequests.createApproval({
    ...requestOptions,
  });
  return data;
};
